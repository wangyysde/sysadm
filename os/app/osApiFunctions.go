...
func validNameHandler(c *sysadmServer.Context) {
	...
	if !islogin {
		response = apiutils.BuildResponseDataForError(7000200001, "您没有登录或者没有权限执行本操作")
	...
	if e != nil || !validName(requestData["objvalue"]) {
		response = apiutils.BuildResponseDataForError(7000200002, "操作系统名称只能由英文和下划线组成，且不得超过10个字符.")
	} else {
