...
func GetMultipartData(c *sysadmServer.Context, keys []string) (map[string]interface{}, error) {
	if c == nil {
		return nil, fmt.Errorf("can not get data on an empty connection")
	}
	...
	if len(keys) < 1 {
		return nil, fmt.Errorf("no key has be specified")
	}

	ret := make(map[string]interface{}, 0)
	formValue := multiform.Value
	formFiles := multiform.File
	for _, k := range keys {
		k = strings.TrimSpace(k)
		value, valueOk := formValue[k]
		if valueOk {
			ret[k] = value
		} else {
			fileHeader, okFile := formFiles[k]
			if okFile {
				ret[k] = fileHeader[0]
			} else {
				ret[k] = []string{}
...
// NewGetRequestData get query data or postform data on a Context for keys([]string)
// the data on Query will be returned if there are the same key on both query and postform.
// return a pointer point to map[string]string which including the data that have be found.
// return nil and error if there is not any data has be found.
func NewGetRequestData(c *sysadmServer.Context, keys []string) (map[string]string, error) {
	ret := make(map[string]string, 0)
	if c == nil {
		return nil, fmt.Errorf("can not get data on nil context")
	}

	if len(keys) < 1 {
		return nil, fmt.Errorf("no keys have be found")
	}
