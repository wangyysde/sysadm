...
func validateOutputFile(cliConf string, fileConf string, envBlock string)string {
	...
	if 	okOutputFile {
		...
		outputFile,e  := sysadmUtils.CheckFileIsRead(envValue,RunConf.WorkingDir)
		if e == nil {
			ret = outputFile
		}
	}
	...
	outputFile,e  := sysadmUtils.CheckFileIsRead(fileConf,RunConf.WorkingDir)
	if e == nil {
		ret = outputFile
	}

	cliConf = strings.TrimSpace(cliConf)
	outputFile,e  = sysadmUtils.CheckFileIsRead(cliConf,RunConf.WorkingDir)
	if e == nil {
		ret = outputFile
	}
