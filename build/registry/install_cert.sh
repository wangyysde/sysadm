...
# Install /etc/sysadm/ssl/{component}/ca.crt to trust CA.
echo "Appending internal tls trust CA to ca-bundle ..."
for caFile in `find /etc/sysadm/ssl -maxdepth 2 -name ca.crt`; do
...
if [[ -d /sysadm_cust_cert && -n "$(ls -A /sysadm_cust_cert)" ]]; then
    echo "Appending trust CA to ca-bundle ..."
    for z in /sysadm_cust_cert/*; do
        case ${z} in
