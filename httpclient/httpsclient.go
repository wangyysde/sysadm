...
// BuildTlsConfWithPEMBlock build tls.Config with certification and key content in PEM for a client connection
func BuildTlsConfWithPEMBlock(caPEM, certPEM, keyPEM []byte, insecureSkipVerify bool) (*tls.Config, error) {
...
// BuildTlsRoundTripper build http.RoundTripper for creating https client. tlsHandshake: TLSHandshakeTimeout
// specifies the maximum amount of time waiting to wait for a TLS handshake. Zero means no timeout.
func BuildTlsRoundTripper(dialer *net.Dialer, tlsConf *tls.Config, tlsHandshake, idleConn, maxIdleConns,
	maxIdleConnsPerHost, maxConnsPerHost, readBuffer, writeBuffer int, disableKeepAlive, disableCompression,
	forceAttempHTTP2 bool) (http.RoundTripper, error) {
