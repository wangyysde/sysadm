...
// then send the response data to the client with specified format.
// if format is not one of the json, html,xml, yaml, then default format is json
func ResponseDataToClient(c *sysadmServer.Context, headers http.Header, status, errorCode int, format string, data interface{}) error {
	if c == nil {
	...
	responseData := ResponseClientData{ErrorCode: errorCode, Data: data}
	switch strings.ToLower(format) {
	case "json":
		c.JSON(status, responseData)
	case "html":
		c.Header("Content-Type", "text/html")
		dataStr := responseData.Data.(string)
		c.String(status, dataStr)
	case "xml":
		c.Header("Content-Type", "text/xml")
		c.XML(status, responseData)
	case "yaml":
		c.Header("Content-Type", "text/yaml")
		c.YAML(status, responseData)
	default:
		c.JSON(status, responseData)
	}
