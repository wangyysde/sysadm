...
var conversionTag = "+sysadm:conversion-gen"
...
func main() {
	...
	for _, pkg := range pkgs {
		...
		sysadmPkg.Name = name
		peerPkgPath, e := getPeerPackageInfo(pkg.Syntax)
		if e != nil {
			fmt.Printf("%s\n", e)
		}
		if peerPkgPath == "" {
		...
		sysadmPkg.InternalTypePath = peerPkgPath
...
func getPeerPackageInfo(syntaxs []*ast.File) (string, error) {
	for _, s := range syntaxs {
		comments := s.Comments
		for _, c := range comments {
			commentStr := strings.TrimSpace(c.Text())
			if strings.HasPrefix(commentStr, conversionTag) {
				tagSlice := strings.Split(commentStr, "=")
				if len(tagSlice) != 2 {
					return "", fmt.Errorf("conversion Tag %s is not valid", commentStr)
				}
				return tagSlice[1], nil
			}
		}
	}
