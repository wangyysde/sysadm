...
type Command struct {
	// object name. the value should be "command"
	Name string
...
// 命令定义表结构
type CommandDefinedSchema struct {
	// 定义的命令ID值
...
// 命令参数定义表结构
type CommandParasDefinedSchema struct {
	// 定义的命令ID值
	ID uint `form:"id" json:"id" yaml:"id" xml:"id" db:"id"`
	// 命令定义表中的命令ID
	CommandID string `form:"commandID" json:"commandID" yaml:"commandID" xml:"commandID" db:"commandID"`
	// 参数类别，具体含义参见command对象的定义文件
	ParaKind int `form:"paraKind" json:"paraKind" yaml:"paraKind" xml:"paraKind" db:"paraKind"`
	// 参数的字段名，当前仅作为标识，后续版本中可考虑在命令执行时将其值传递给命令
	Key string `form:"key" json:"key" yaml:"key" xml:"key" db:"key"`
	// 当参数类别是固定类别时，此值即为参数值
	Value string `form:"value" json:"value" yaml:"value" xml:"value" db:"value"`
	// 当参数类别为从对象表中获取时，本字段为指定对象的表名称
	TableName string `form:"tableName" json:"tableName" yaml:"tableName" xml:"tableName" db:"tableName"`
	// 当参数类别为从对象表中获取时，本字段为指定对象表内记录的主键值
	PkName string `form:"pkName" json:"pkName" yaml:"pkName" xml:"pkName" db:"pkName"`
	// 当参数类别为从对象表中获取时，表示取以本字段值为字段名的对象表内字段值作为参数值
	FieldName string `form:"fieldName" json:"fieldName" yaml:"fieldName" xml:"fieldName" db:"fieldName"`
	// 当参数类别为通过另一个命令获取时,本字段指定的是获取参数值的另一个命令的commandDefined表中对应ID值
	SubCommandID int `form:"subCommandID" json:"subCommandID" yaml:"subCommandID" xml:"subCommandID" db:"subCommandID"`
}

// 存储节点上要执行和正在执行的命令的信息。执行完的命令信息自动移到commandHistory表中
type CommandSchema struct {
	// commandID identified a command
	CommandID string `form:"commandID" json:"commandID" yaml:"commandID" xml:"commandID" db:"commandID"`
	// 命令定义表中的ID值
	DefinedID uint `form:"definedID" json:"definedID" yaml:"definedID" xml:"definedID" db:"definedID"`
	// 命令是否依赖于其它命令，0表示不依赖于其它命令的独立命令,否则本字段记录被依赖命令的commandID
	DependendID string `form:"dependendID" json:"dependendID" yaml:"dependendID" xml:"dependendID" db:"dependendID"`
	// 0表示内嵌命令,1系统命令, 2 脚本或批处理程序
	Type int `form:"type" json:"type" yaml:"type" xml:"type" db:"type"`
	// 命令事务范围,当dependent值为0时忽略本字段值.含义见command对象定义文件
	TransactionScope int `form:"transactionScope" json:"transactionScope" yaml:"transactionScope" xml:"transactionScope" db:"transactionScope"`
	// 命令的执行是否必须带至少一个参数，0表示否，1表示是
	MustParas int `form:"mustParas" json:"mustParas" yaml:"mustParas" xml:"mustParas" db:"mustParas"`
	// command name , agent will route handler according to the value of this filed
	Command string `form:"command" json:"command" yaml:"command" xml:"value" db:"command"`
	// identifying which host
	HostID uint `form:"hostID" json:"hostID" yaml:"hostID" xml:"hostID" db:"hostID"`
	// 如果命令是crontab类别的自动执行命令，本字段指定crontab格式的执行时间和周期
	Crontab string `form:"crontab" json:"crontab" yaml:"crontab" xml:"crontab" db:"crontab"`
	// 指示命令是否是属地同步命令。所谓同步命令是指，命令能够快速执行完成，即能在一个HTTP会话请求超时之前（一般超时时间为几秒内）执行完成的命令。0表示同步命令，否则为异步命令
	Synchronized int `form:"synchronized" json:"synchronized" yaml:"synchronized" xml:"synchronized" db:"synchronized"`
	// the time when the command  has be added
	CreateTime int `form:"createTime" json:"createTime" yaml:"createTime" xml:"createTime" db:"createTime"`
	// the time when the command has be has be sent to agent
	SendTime int `form:"sendTime" json:"sendTime" yaml:"sendTime" xml:"sendTime" db:"sendTime"`
	// the time when the command has  run
	CompleteTime int `form:"completeTime" json:"completeTime" yaml:"completeTime" xml:"completeTime" db:"completeTime"`
	// how many times has be tried on the current stage
	TryTimes int `form:"tryTimes" json:"tryTimes" yaml:"tryTimes" xml:"tryTimes" db:"tryTimes"`
	// command status code
	Status int `form:"status" json:"status" yaml:"status" xml:"status" db:"status"`
}

type CommandParametersSchema struct {
	// parametersID identified a parameter
	ParametersID uint `form:"parametersID" json:"parametersID" yaml:"parametersID" xml:"parametersID" db:"parametersID"`
	// 对应于commandParasDefined表中的key
	Name string `form:"name" json:"name" yaml:"name" xml:"name" db:"name"`
	// 对应于commandParasDefined表中的value
	Value string `form:"value" json:"value" yaml:"value" xml:"value" db:"value"`
	// commandID identified a command
	CommandID string `form:"commandID" json:"commandID" yaml:"commandID" xml:"commandID" db:"commandID"`
	// 参数类别，具体含义参见command对象的定义文件
	ParaKind int `form:"paraKind" json:"paraKind" yaml:"paraKind" xml:"paraKind" db:"paraKind"`
	// 当参数类别为从对象表中获取时，本字段指定获取参数值的对象表中字段名称
	SubCommandID string `form:"subCommandID" json:"subCommandID" yaml:"subCommandID" xml:"subCommandID" db:"subCommandID"`
}
